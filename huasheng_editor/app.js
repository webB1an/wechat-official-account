...
const editorApp = createApp({
  ...
  methods: {
    ...
    visualStyle(brief) {
      ...
      const textColor = palette[0] || '#17202A';
      const backgroundColor = palette[1] || '#F6F1E8';
      const accentColor = palette[2] || '#C8A96A';
      return {
        color: textColor,
        background:
          `radial-gradient(circle at 78% 24%, ${accentColor}55 0, transparent 28%), ` +
          `linear-gradient(135deg, ${backgroundColor} 0%, #ffffff 100%)`
      };
    },

    async generateBookArticle() {
      if (this.generatingArticle) return;
      this.generatingArticle = true;
      this.showToast('正在生成随机推书文章...', 'success');
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(payload.message || payload.error || '生成失败');
        }

        this.generatedArticle = payload.articlePackage;
        this.generationTheme = payload.theme || '';
        this.isMockGeneration = Boolean(payload.mock);
        this.coverImageDataUri = '';
        this.inlineImageDataUris = [];
        this.currentGeneratedBookTitle = this.generatedArticle.book.title;

        await this.$nextTick();
        await this.renderGeneratedImages();

        this.markdownInput = this.buildGeneratedMarkdown(
          this.generatedArticle,
          this.inlineImageDataUris
        );
        this.currentArticleId = null;
        this.showToast('文章和配图已生成', 'success');
      } catch (error) {
        console.error('生成随机推书失败:', error);
    ...
    async renderGeneratedImages() {
      ...
      const coverNode = this.$refs.coverVisualNode;
      const inlineNodes = Array.isArray(this.$refs.inlineVisualNodes)
        ? this.$refs.inlineVisualNodes
        : (this.$refs.inlineVisualNodes ? [this.$refs.inlineVisualNodes] : []);

      if (!coverNode || inlineNodes.length !== 3) {
        throw new Error('图片模板尚未准备好');
      }
      ...
      this.coverImageDataUri = await window.htmlToImage.toPng(coverNode, {
        ...imageOptions,
        canvasHeight: 383
      });
      ...
      for (const node of inlineNodes) {
        imageDataUris.push(await window.htmlToImage.toPng(node, {
          ...imageOptions,
          canvasHeight: 506
        }));
      }
      this.inlineImageDataUris = imageDataUris;
    },
    ...
    buildGeneratedMarkdown(articlePackage, inlineImages) {
      ...
      const imageQueue = [...inlineImages];
      const imageSlots = new Set();
      if (sections.length > 0) imageSlots.add(0);
      if (sections.length > 2) imageSlots.add(Math.floor(sections.length / 2));
      if (sections.length > 3) imageSlots.add(Math.max(0, sections.length - 2));

      let visibleTitleCount = 0;
      sections.forEach((section, index) => {
        const shouldRenderTitle = section.title && visibleTitleCount < 2 && !this.isTemplateSectionTitle(section.title);
        ...
        if (section.items && section.items.length > 0) lines.push('');
        ...
        if (section.callToAction) {
          lines.push(section.callToAction, '');
        }

        if (imageSlots.has(index) && imageQueue.length > 0) {
          const imageIndex = inlineImages.length - imageQueue.length + 1;
          lines.push(`![文内图${imageIndex}](${imageQueue.shift()})`, '');
        }
      });

      if (articlePackage.article?.closingCallToAction) {
        lines.push(articlePackage.article.closingCallToAction, '');
      }

      if (Array.isArray(articlePackage.interactionPrompts) && articlePackage.interactionPrompts.length > 0) {
        const prompt = articlePackage.interactionPrompts[0];
        if (prompt && prompt !== articlePackage.article?.closingCallToAction) {
          lines.push(prompt, '');
        }
      }

      if (Array.isArray(articlePackage.tags) && articlePackage.tags.length > 0) {
        lines.push(articlePackage.tags.map((tag) => `#${tag}`).join('  '), '');
      }
    ...
    getGeneratedArticleTitle() {
      return this.generatedArticle?.titles?.[0]?.title || this.generatedArticle?.book?.title || '';
    },

    isTemplateSectionTitle(title) {
      const templateTerms = [
        '核心洞察',
        '为什么值得',
        '值得每个人读',
      ...
      return templateTerms.some((term) => title.includes(term));
    },

    downloadCoverImage() {
      if (!this.coverImageDataUri) {
        this.showToast('还没有封面图可下载', 'error');
        return;
      }

      const safeTitle = (this.currentGeneratedBookTitle || '随机推书')
        .replace(/[\\/:*?"<>|]/g, '')
        .slice(0, 40);
      const link = document.createElement('a');
      link.href = this.coverImageDataUri;
      link.download = `公众号封面-${safeTitle}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      this.showToast('封面图已下载', 'success');
    },

    async markCurrentBookCopied() {
      if (!this.currentGeneratedBookTitle) return;

      try {
        const response = await fetch('/api/mark-copied', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ bookTitle: this.currentGeneratedBookTitle })
        });
        if (!response.ok) {
        ...
        console.error('标记推过失败:', error);
        this.showToast('复制成功，但标记推过失败: ' + error.message, 'error');
      }
    },
    ...
    async copyToClipboard() {
      ...
      let fallbackHTML = this.renderedContent;
      ...
      try {
        ...
        if (document.hasFocus()) {
          ...
          // 自动保存到历史记录
          this.saveToHistory();
          await this.markCurrentBookCopied();
